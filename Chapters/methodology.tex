...
four-qubit case. Each qubit starts in $|0\rangle$. Within every layer the features
$x_q$ are re-uploaded through $R_Y(x_q)$ rotations, followed by a trainable
single-qubit rotation $\mathrm{Rot}(\theta^{(\ell)}_q)$ with three angles per qubit
per layer, and a ring of CNOT gates. The dashed block is repeated $L$ times, and
each qubit is measured in the Pauli-$Z$ basis. The sweep varies
$(n_\text{qubits}, n_\text{layers})$ over $(2,12),(3,8),(4,6),(6,4),(8,3)$, all at a
constant $n_\text{qubits}\cdot n_\text{layers}\cdot 3 = 72$ trainable angles.}
